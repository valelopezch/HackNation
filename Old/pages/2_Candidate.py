...
code = st.text_area("Paste your fixed snippet here", height=220)
...
for i, (img, correct) in enumerate(PLOTS.items()):
    with cols[i]:
        ...
        answers[img] = st.selectbox(f"Model {i+1}", ["", "Logistic Regression", "kNN", "Decision Tree", "SVM (RBF)"], key=f"m{i}")
